...
PRICING_PER_1K_TOKENS = {
    ...
    # "gemini-3.8-flash": {"input": 0.0025, "output": 0.010},
    # "gemini-3.6-flash": {"input": 0.00015, "output": 0.0006},
    # "gemini-3-flash": {"input": 0.00075, "output": 0.00375},
...
FLASH_MODEL = os.getenv("LAB_MODEL", "gemini-3.8-flash")
FLASH_LITE_MODEL = os.getenv("LAB_MINI_MODEL", "gemini-3.5-flash-lite")
OPENAI_MINI_MODEL = FLASH_LITE_MODEL
...
def call_openai(
    ...
    client = OpenAI(
        api_key=os.getenv("LAB_API_KEY", os.getenv("OPENAI_API_KEY")),
        base_url=os.getenv(
            "LAB_BASE_URL",
...
def call_openai_mini(
    ...
    return call_openai(prompt=prompt, model=FLASH_LITE_MODEL, temperature=temperature, top_p=top_p, max_tokens=max_tokens)
...
def compare_models(prompt: str) -> dict:
    ...
    f_text, f_latency = call_openai(prompt)
    fl_text, fl_latency = call_openai_mini(prompt)
        
    cost = (len(f_text.split()) / 0.75) / 1000 * PRICING_PER_1K_TOKENS["gpt-4o"]["output"]
    
    return {"gpt4o_response": f_text,       #f_response
            "mini_response": fl_text,       #fl_response
            "gpt4o_latency": f_latency,     #f_latency
            "mini_latency": fl_latency,     #fl_latency
            "gpt4o_cost_estimate": cost}    #flash_code
...
# ---------------------------------------------------------------------------
def count_tokens(text: str, model: str = FLASH_MODEL) -> int:
    """
...
# ---------------------------------------------------------------------------
def estimate_cost(prompt: str, response: str, model: str = FLASH_MODEL) -> dict:
    """
...
def streaming_chatbot() -> None:
    ...
    client = OpenAI(
            api_key=os.getenv("LAB_API_KEY", os.getenv("OPENAI_API_KEY")),
            base_url=os.getenv("LAB_BASE_URL","https://generativelanguage.googleapis.com/v1beta/openai/")
            )
    history = []
    reply = ""
    ...
    while True:
        ...
        stream = client.chat.completions.create(model=FLASH_MODEL, messages=messages, stream=True)
...
def run_assistant(
    ...
    while True:
        ...
        stream = retry_with_backoff(lambda: client.chat.completions.create(model=FLASH_MODEL, messages=messages, stream=True))
